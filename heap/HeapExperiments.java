...
public class HeapExperiments
{
   ...
   public static void experiments(int population, boolean iteratedInsertion, boolean worstCase, boolean changeKey) {
      ...
      {
    	  	startTime = System.nanoTime();
    	  	makeChanges(students, heap, population);
    	  	endTime = System.nanoTime();
    	  	duration = endTime - startTime;
   ...
   private static void makeChanges(ArrayList<Student> students, MaxHeap heap, int size)
   {
      for(int i = 0; i < CHANGES; i++)
      {
         int index = (int) (Math.random() * size);
         Student s = students.get(index);
         double newClassGrade = Math.random() * 4;
         int units = (int) (1 + Math.random() * 6);
         heap.addGrade(s, newClassGrade, units);
      }
   }
   ...
   private static ArrayList<Student> createStudents(int number, boolean worstCase)
   {
      ...
      for(int i=0; i<number; i++)
      {
         ...
         students.add(new Student("student" + i, gradePoints, units));
      }
      return students;
   }
